class RechargeWidget extends HTMLElement {
    ...
    async renderNextShipment(sub) {
      const existingNextShipmentEl = document.querySelector('.next-shipment');
      
      if (!existingNextShipmentEl) {      
        ...
        nextShipmentEl.innerHTML += `
            <button class="next-shipment__add btn" data-subscription-id="${sub.id}">Add to next shipment</button>
            <span class="next-shipment__title">Your next shipment is scheduled for ${nextDate}.</span>
        `;

        document.querySelector('.payment-buttons').appendChild(nextShipmentEl);
    ...
    async addToNextShipment(sub) {
        let variantId  = document.querySelector('.product-variant-id').value,
            productId = document.querySelector('.widget').getAttribute('data-product-id'),
        ...
        window.location = '/tools/recurring/login'
    }
